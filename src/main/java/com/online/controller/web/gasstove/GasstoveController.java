package com.online.controller.web.gasstove;
...
@Controller("gasstoveController")
@RequestMapping("/web/gasstove")
public class GasstoveController extends BaseController {
	...
	public String simple_list(ModelMap model) {
		...
		FieldModule fieldModule = fieldModuleService.findByNameTableType(Constants.FIELD_detail_gasstove);
		if(fieldModule != null ){
	...
	public String updatePage(ModelMap model,Integer id) {
		model.put("gasStove", gasStoveService.find(id));
		Project  project = SpringUtils.getCurrentProject();
		model.put("project", project);
	...
	@ResponseBody
	public Page<GasStove> query(Pageable pageable,BoilerInformation boilerInformation) {
		...
		return gasStoveService.findPage(pageable);
	...
	@ResponseBody
	public Message save(GasStove gasStove, Integer exhaustionHoleId) {
		gasStoveService.saveGasStove(gasStove,exhaustionHoleId);
	...
	@ResponseBody
	public Message update(GasStove gasStove,Integer exhaustionHoleId) {
		gasStoveService.updateGasStove(gasStove,exhaustionHoleId);
		return Message.success();
	}
	...
	public @ResponseBody Message delete(Integer... ids) {
		for (int i = 0; i < ids.length; i++) {
			gasStoveService.delete(ids[i]);
			monthlyInformationService.deleteByType(Constants.MONTH_TYPE_GASSTOVE,ids[i]);
		}
		return Message.success();
	}
