...
(function (a) {
	...
	a.extend(a.validator, {
		...
		messages : {
		    ...
		    maxlength: a.validator.format("最多{0}位"),   
		    minlength: a.validator.format("最少{0}位"),   
		    rangelength: a.validator.format("请输入一个长度介于 {0} 和 {1} 之间的字符串"),   
		    range: a.validator.format("{0}-{1}"),   
		    max: a.validator.format("最大为 {0}"),  
		    min: a.validator.format("最小为 {0}")
		},
		autoCreateRanges : !1,
		prototype : {
			init : function () {
		...
		methods : {
			...
			remote : function (b, c, d) {
				...
				d = "string" == typeof d && {
				 ...
				 || d,
				g.old === b ? g.valid : (g.old = b, e = this, this.startRequest(c), f = {}, f[c.name] = b, a.ajax(a.extend(!0, {
							url : d,
							mode : "abort",
							...
							success : function (d) {
								...
								j ? (i = e.formSubmitted, e.prepareElement(c), e.formSubmitted = i, e.successList.push(c), delete e.invalid[c.name], e.showErrors()) : (f = {}, h = d || e.defaultMessage(c, "remote"), f[c.name] = g.message = a.isFunction(h) ? h(b) : h, e.invalid[c.name] = !0, e.showErrors(f)),
								g.valid = j,
								e.stopRequest(c, j)
							}
						}, d)), "pending")
	...
	jQuery.validator.addMethod("mins",function(value,element,params){
		if(value>params){
			return true;
		}
	},"值必须大于{0}");
	...
	//校验单位不能为数字，只能是汉字或字母
	jQuery.validator.addMethod("checkUnit", function(value, element) {       
     	return this.optional(element) || /^\D+$/.test(value);       
	}, "字符或符号");
});
