...
<script type="text/javascript">
	require(["grid","hide","select","panel"],function(g,hide){
		var opts = {
				...
				url:"query.jhtml",
				columns :[
					{name:"solventType",caption:"溶剂类型",width:60/5,hidden:false,type:"text"},
					{name:"solventNature",caption:"溶剂性质",width:60/5,hidden:false,type:"text"},
					{name:"vocsContent",caption:"VOCs含量（%）",width:60/5,hidden:false,type:"text"},
					{name:"vocsVolatility",caption:"VOCs挥发度（%）",width:60/5,hidden:false,type:"text"},
					{name:"annual",caption:"年用量",width:60/5,hidden:false,type:"text"},
		...
		$('#delete').click(function(e){//删除
			if($(this).attr("disabled")) return;
			qy.sure({
				title:'是否删除所选'+grid1.getSelectedValue('id').length+'条有机溶剂使用情况?',
				yes:function(index){
					layer.close(index);
