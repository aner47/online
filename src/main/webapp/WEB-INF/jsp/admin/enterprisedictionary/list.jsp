...
require(["grid","select","panel","less","upload"],function(g,s,p,less,upload){
	...
	var projectSelect = $("#status").select({name:"status"})	
	...
	var projectIdSelect = $("#projectId").select({name:"projectId"})	
		...
		$("#filter").click(function(){
			layer.confirm('开始筛选将清空上次筛选结果?', {icon: 2, title:'系统提示信息',cancel:function(index){layer.close(index);}}, function(index){
	    		layer.close(index);
		...
		var opts = {
				...
				columns :[
					...
					{name:"alreadyEnterpriseId",caption:"已录入企业id",hidden:false,type:"text",format:function(value){return value==-1?"":value}},
				    {name:"sign",caption:"标记项",hidden:false,type:"text"},
...
<body>
		...
		<h1 class="title">
		 ...
		 <div class="form-line form-line2">
            <form id="queryForm" action="">
                ...
                <div class="group col-lg-5f1 col-md-4 unmust">
                    <label class="form-label" for="enterpriseName">企业名称</label> 
                   <input id="enterpriseName" name="enterpriseName"/>
                </div>
                <div class="group col-lg-5f1 col-md-4 unmust">
                    <label class="form-label" for="countyName">县</label> 
                   <input id="countyName" name="countyName"/>
                </div>
                <div class="group col-lg-5f1 col-md-4 unmust">
                    <label class="form-label" for="countyName">状态</label> 
                   <div id="status" class="selectF" data-code="名录库【状态】" ></div>
                </div>
                <div class="group col-lg-5f1 col-md-4 unmust">
                    <label class="form-label" for="countyName">标记项</label> 
	...
	<div class="grid-tools">
		...
		<button id="insert" class="btn-denger" data-grid="tab1"
			data-enable="3">生成任务</button>
		<button id="filter" class="btn-denger" data-grid="tab1"
			data-enable="4">筛选重复</button>
		<button id="filterResult" class="btn-denger" data-grid="tab1"
			data-enable="6" style="width: 120px">筛选重复结果</button>
		<button id="export" class="btn-denger" data-grid="tab1"
			data-enable="5">导出</button>
		<button id="batchCheck" class="btn-denger" data-grid="tab1"
			data-enable="7">批量审核</button>
	...
	<div class="form-row">
		...
		<div class="bottonCatain">
			<a href="../enterprisedictionary/downloanExcel.jhtml">模板下载</a>
		</div>
		</div>
	</div>
	<div class="repeatBlock"></div>
</body>
</html>