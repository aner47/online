...
require(["grid","select","panel","less"],function(g){
		$('#delete').click(function(e){//删除
			layer.confirm('是否删除所选'+grid1.getSelectedValue('id').length+'条产品标准名称配置信息?', {icon: 2, title:'系统提示信息',cancel:function(index){layer.close(index);}}, function(index){
	    		layer.close(index);
				...
				var opts ={
					...
					callBack:function(data,errcode){
						if(errcode!=="000000"){
							layer.msg('删除产品标准名称配置成功！', {icon: 6});
							grid1.loadData();
						}else{
							layer.msg('删除产品标准名称配置失败！', {icon: 5});
						}
					}
				}
				qy.ajax(opts);
		...
		var opts = {
				...
				columns :[
                    ...
                    {name:"standardName",caption:"产品名称",hidden:false,type:"text"},
                    {name:"industryCode",caption:"行业代码",hidden:false,type:"text"},
                    {name:"standardUnit",caption:"产品标准单位",hidden:false,type:"text"},
                    {name:"defaultUnit",caption:"常用转换单位",hidden:false,type:"text"},
                    {name:"defaultConversionFactor",caption:"常用转换系数",hidden:false,type:"text"},
                    {name:"pscc",caption:"源分类",hidden:false,type:"text",valueSet:"污染源分类"},
...
<body>
    <div class="block ">
        ...
        <form id="queryForm" action="">
            ...
            <label class="form-label" for="standardName">产品名称</label>
                <input type="search" name="standardName" id="standardName" class="input" >
            </div>
