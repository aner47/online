...
<script type="text/javascript">
	require(["grid","hide","select","panel"],function(g,hide){
		var opts = {
				...
				url:"query.jhtml",
				columns :[
					{name:"num",caption:"数量",width:60/5,hidden:false,type:"text"},
					{name:"materialName",caption:"运输物料名称",width:60/5,hidden:false,type:"text"},
					{name:"carType",caption:"运输车辆类型",width:60/5,hidden:false,type:"text"},
					{name:"dischargeStandard",caption:"运输车排放标准",width:60/5,hidden:false,type:"text"},
					{name:"fuelType",caption:"燃料类型",width:60/5,hidden:false,type:"text"},
					{name:"transportNumber",caption:"运输车次（次/月）",width:60/5,hidden:false,type:"text"},
					...
					{name:"oilWear",caption:"单车油耗量(升/公里)",width:60/5,hidden:false,type:"text"},
					{name:"source",caption:"主要来源地或目的地",width:60/5,hidden:false,type:"text"},
					{name:"avgDistance",caption:"平均单次运距（km）",width:60/5,hidden:false,type:"text"},
					{name:"ratedLoad",caption:"额定载重(t)",width:60/5,hidden:false,type:"text"},
					{name:"avgLoad",caption:"单次平均载重（吨）",width:60/5,hidden:false,type:"text"},
		...
		$('#delete').click(function(e){//删除
			if($(this).attr("disabled")) return;
			qy.sure({
				title:'是否删除所选'+grid1.getSelectedValue('id').length+'条普查表运输车信息?',
				yes:function(index){
					layer.close(index);
...
<body>
	<div class="block">
		<h1 class="title">
			<i class="iconfont icon-jidongcheshuliang"></i>运输车信息
