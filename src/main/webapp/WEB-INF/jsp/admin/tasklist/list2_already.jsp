...
require(["grid","select","panel","less"],function(g){
        ...
        $("#AuditorPage1").click(function(){
            var opts = {
                title: "任务审核",
                url : "chooseReviewTask.jhtml",
        ...
        var opts = {
                ...
                events:{
                	rowSelected:function(handle){
						...
						$.each(rowData2,function(i,v){
							if(v.checkAccount != currentId){
								$("#AuditorPage1").attr("disabled","disabled");
							}
						})
                ...
                columns :[
				    ...
				    {name:"status",caption:"状态",hidden:false,type:"text",valueSet:"每日任务清单【任务状态】"},
                     ...
                     {name:"checkAccount",caption:"任务复核员",type:"text",hidden:true,format:function(value,rowValue,handle){
				    	var currentId = $("#currentUser").val();
						if(value != currentId){
...
<body>
    ...
    <div class="grid-tools">
    	<c:forEach items="${currentUserRoles}" var="item">
			<c:if test="${item.name == '每日任务审核员'}">
				<!-- <button id="AuditorPage1" class="btn-warning" data-grid="tab1"   data-enable="2">审核</button> -->
			</c:if>
		</c:forEach>
    </div>
    <div id="tab1"></div>
