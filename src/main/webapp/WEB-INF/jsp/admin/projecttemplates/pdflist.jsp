...
require(["grid","select","panel","less","progresbar"],function(g){
	$("#queryForm #templateType").select({filter:true,name:"templateType",zIndex:3,change:function(event, value){}});
		...
		var opts = {
				tab:"tab1",
				data:{pageSize:10,pageNumber:0,templatesType:"pdf"},
				url:"query.jhtml",
				columns :[
						  {name:"id",caption:"ID",hidden:true,type:"text"},
						  {name:"name",caption:"表格模块名称",hidden:false,type:"text"},
                          ...
                          {name:"operate",caption:"操作",isHtml:true,html:'<a id="view"  style="cursor: pointer;"><i class="iconfont icon-xiangqing"></i></a>',
                              events :[{
                                       ...
                                       handle:function(){
                                             ...
                                             location.href = "../projectmoduleinfo/list.jhtml?templateId="+id+"&source=pdflist";
                                       }
                                     }
                              ]
                       } 
