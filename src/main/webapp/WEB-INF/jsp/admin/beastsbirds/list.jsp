...
<script type="text/javascript">
	require(["grid","hide","select","panel"],function(g,hide){
		var opts = {
				...
				url:"query.jhtml",
				columns :[
					{name:"kind",caption:"畜禽种类",width:60/5,hidden:false,type:"text"},
					{name:"feedingNum",caption:"饲养量（只）",width:60/5,hidden:false,type:"text"},
					{name:"saveNum",caption:"存栏数（只）",width:60/5,hidden:false,type:"text"},
					{name:"outNum",caption:"出栏数（只）",width:60/5,hidden:false,type:"text"},
					{name:"feedingPeriod",caption:"饲养周期（天）",width:60/5,hidden:false,type:"text"},
					{name:"feedingIndoorProportion",caption:"饲养室内比例（%）",width:60/5,hidden:false,type:"text"},
					{name:"process",caption:"尿液粪便处理方式",width:60/5,hidden:false,type:"text"},
					{name:"fodderProportion",caption:"粪肥作饲料的比重（%）",width:60/5,hidden:false,type:"text"},
		...
		$('#delete').click(function(e){//删除
			if($(this).attr("disabled")) return;
			qy.sure({
				title:'是否删除所选'+grid1.getSelectedValue('id').length+'条畜禽养殖信息?',
				yes:function(index){
					layer.close(index);
