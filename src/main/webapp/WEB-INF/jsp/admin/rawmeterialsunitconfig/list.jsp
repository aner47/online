...
require(["grid","select","panel","less"],function(g){
		$('#delete').click(function(e){//删除
			layer.confirm('是否删除所选'+grid1.getSelectedValue('id').length+'条原辅料单位配置信息?', {icon: 2, title:'系统提示信息',cancel:function(index){layer.close(index);}}, function(index){
	    		layer.close(index);
				...
				var opts ={
					...
					callBack:function(data,errcode){
						if(errcode!=="000000"){
							layer.msg('删除原辅料单位配置成功！', {icon: 6});
							grid1.loadData();
						}else{
							layer.msg('删除原辅料单位配置失败！', {icon: 5});
						}
					}
				}
				qy.ajax(opts);
		...
		var opts = {
				...
				url:"query.jhtml",
				columns :[
                    {name:"rawMaterialsOtherName.otherName",caption:"名称",hidden:false,type:"text"},
