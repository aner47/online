...
require(["validate","select","ajaxform"],function(a){
		...
		var project1Select = $("#projectId","#inputForm").select({name:"projectId",defaultValue:'${pdfTemplateFile.projectId}',zIndex:10,change:function(event, value){
			projectTemplateName.loadData("projectId:"+value+",templatesType:pdf");
		}});
		...
		var projectTemplateName = $("#inputForm #projectTemplateName").select({filter:true,param:"templatesType:pdf",name:"projectTemplateName",defaultValue:'${pdfTemplateFile.projectTemplateName}',zIndex:6,change:function(event, value){}});
		if('${pdfTemplateFile.projectId}'){
			projectTemplateName.loadData("projectId:"+'${pdfTemplateFile.projectId}'+",templatesType:pdf");
		}
