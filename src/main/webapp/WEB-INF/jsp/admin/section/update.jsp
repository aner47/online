...
<script type="text/javascript">
	require(["hide","validate","ajaxform","panel","date","year"],function(hide){
	    ...
	    $("#stopDate", "#section_form").jeDate({
	     ...
	     /* 日期比较-开始  */
				...
				/* 日期比较-结束 */
		$("#useStatus","#section_form").select({filter: false,name:"useStatus",zIndex:19,isCustom : true,defaultValue:'${section.useStatus}'});
	    $("#facilityUnit","#section_form").select({filter: false,name:"facilityUnit",zIndex:18,isCustom : true,defaultValue:'${section.facilityUnit}'});
		$("#productUnit","#section_form").select({filter: false,name:"product.unit",zIndex:17,isCustom : true,defaultValue:'${section.product.unit}'});
	    
		$("#fuelType", "#section_form").select({name : "fuelType",isCustom : true,zIndex : 16,defaultValue:'${section.fuelType}',
	  		validate : 'required'
	  	});
	  	$("#fuelUnit", "#section_form").select({name : "fuelUnit",isCustom : true,zIndex : 15,defaultValue:'${section.fuelUnit}',
	  		validate : 'required'
	  	});
	  	
		$("#contaminantType","#section_form").select({filter: false,name:"contaminantType",zIndex:14,multselect: true,defaultValue:'${section.contaminantType}',isCustom : true});
		//$("#dischargeModality","#section_form").select({filter: false,name:"dischargeModality",zIndex:5,defaultValue:'${section.dischargeModality}'});
	
		var simpleData = {};
		 simpleData['-1'] ='无组织排放';
		 $("#dischargeModality","#section_form").select({filter: false,name:"dischargeModality",zIndex:13,defaultValue:'${section.dischargeModality}',change:function(event,value){
			if(value=='无组织'){
				console.log('无组织');
		...
		$("#governanceMeasures1","#section_form").select({name:"governanceMeasures1",zIndex:12,defaultValue:defaultgovernanceMeasures1});
		$("#governanceMeasures2","#section_form").select({name:"governanceMeasures2",zIndex:11,defaultValue:defaultgovernanceMeasures2});
		$("#governanceMeasures3","#section_form").select({name:"governanceMeasures3",zIndex:10,defaultValue:defaultgovernanceMeasures3});
		$("#governanceMeasures4","#section_form").select({name:"governanceMeasures4",zIndex:9,defaultValue:defaultgovernanceMeasures4});
		//$("#exhaustionHoleId","#section_form").select({name:"exhaustionHoleId",zIndex:1,defaultValue:'${section.enterpriseEmissionsManagement.exhaustionHole.id}'});
		if('${section.dischargeModality}'=='无组织'){
			console.log('无组织');
			console.log('${section.enterpriseEmissionsManagement.exhaustionHole.id}');
			// exhaustionHoleId.loadDataSimpleData(simpleData);
			 var exhaustionHoleId = $("#exhaustionHoleId","#section_form").select({simpleData:simpleData,name:"exhaustionHoleId",zIndex:8,defaultValue:'-1'});
		 }else{
			 var exhaustionHoleId = $("#exhaustionHoleId","#section_form").select({name:"exhaustionHoleId",zIndex:8,defaultValue:'${section.enterpriseEmissionsManagement.exhaustionHole.id}'});
		 }
...
<body>
	...
	<form class="hideCls" id="section_form" style="padding:0;width:100%;">
		...
		<div class="infLine">
			...
			<div class="infPara">
				<div class="infParaCon">
					<label for="" class="infpLable"><span style="margin-right: 5px;color: #eb2a33;">*</span>产品产量</label>
					<div class="infpCon">
			...
			<div class="infPara">
				<div class="infParaCon">
					<label for="" class="infpLable"><span style="margin-right: 5px;color: #eb2a33;">*</span>设计产能</label>
					<div class="infpCon">
			...
			<!-- <div class="infPara">
				<div class="infParaCon">
					<label for="" class="infpLable">治理措施编号4</label>
					<div class="infpCon">
						<div id="governanceMeasures4" data-code="污染治理措施" class="selF"></div>
					</div>
				</div>
