...
<script type="text/javascript">
	require(["grid","hide","select","panel"],function(g,hide){
		var opts = {
				...
				url:"query.jhtml",
				columns :[
					{name:"mechanical",caption:"机械",width:60/5,hidden:false,type:"text"},
					{name:"oilStandard",caption:"油品标准",width:60/5,hidden:false,type:"text"},
					{name:"avgRate",caption:"机械平均功率（千瓦/辆）",width:60/5,hidden:false,type:"text"},
					{name:"avgConsumption",caption:"单车油耗量",width:60/5,hidden:false,type:"text"},
					{name:"avgConsumptionUnit",caption:"单位",width:60/5,hidden:false,type:"text"},
		...
		$('#delete').click(function(e){//删除
			if($(this).attr("disabled")) return;
			qy.sure({
				title:'是否删除所选'+grid1.getSelectedValue('id').length+'条机械消耗信息?',
				yes:function(index){
					layer.close(index);
...
<body>
	<div class="block">
		<h1 class="title">
			<i class="iconfont icon-gongshuaixiaohao"></i>
			机械消耗信息
			<c:if test="${principal.userType != 'system'}">
			<div id="back" class="titleBtn">返回</div>
			<div id="batchadd" class="titleBtn">批量添加</div>
